...
export default function BreakdownScreen({ route }) {
  const { contributors, itemsData } = route.params;

  return (
    <View>
      {contributors.map((contributor) => (
        <View key={contributor}>
          <Text>{contributor}</Text>
          {itemsData.map(item => (
            item.pricePerContributor[contributor] ? (
              <Text key={item.itemName}>
                {item.itemName}: ${item.pricePerContributor[contributor].toFixed(2)}
              </Text>
            ) : null
          ))}
        </View>
...
//        <NavigationBar title="Your Items" />
